...
class LanguageManager:
    ...
    def _load_translations(self):
        ...
        # Determine base path (handle PyInstaller bundle)
        if hasattr(sys, '_MEIPASS'):
            base_path = sys._MEIPASS
        else:
            base_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
        
        language_dir = os.path.join(base_path, "src", "core", "language")
        
        # Alternative path for different structures
        if not os.path.exists(language_dir):
            if hasattr(sys, '_MEIPASS'):
                 # Check flat structure in bundle
                 language_dir = os.path.join(base_path, "language")
            else:
